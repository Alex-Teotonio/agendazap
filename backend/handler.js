...
// Garante que o nutricionista exista (marca role = 'nutricionista' se ainda n�o tiver)
async function ensureNutricionistaExists(userId) {
  // tenta buscar pelo PK real (userId)
  const { Item } = await dynamo.get({
    TableName: usersTable,
    Key: { userId },
  }).promise();

  // se existir mas n�o tiver role, adiciona
  if (Item && Item.role !== 'nutricionista') {
    await dynamo.update({
      TableName: usersTable,
      Key: { userId },
      UpdateExpression: "SET role = :r",
      ExpressionAttributeValues: { ":r": "nutricionista" }
    }).promise();
  }
}
...
app.post("/login", [
  ...
  try {
    ...
    const token = jwt.sign(
      { userId: user.userId, email: user.email, role: "nutricionista" },
      process.env.JWT_SECRET,
      { expiresIn: "1h" }
    );
  
    await ensureNutricionistaExists(user.userId);
...
// no topo do arquivo, ap�s definir `authenticate`:
app.use(authenticate);
